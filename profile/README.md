...
### 공격 경로 예측
기존 인프라를 그래프 형태로 모델링한 후, 생성 예정인 인프라를 가상 노드로 추가하여 연결 관계를 확장합니다. 이후 새롭게 도출된 인프라 노드의 접근 경로를 임베딩하여, 공격 시나리오가 저장된 벡터 DB와 RAG 방식으로 비교·분석하여 수행됩니다.

### 최소 권한 IAM 정책
AI가 제안한 정책은 사용자의 최종 승인 절차를 거칩니다. 이후 자격 증명 생성 → 인프라 생성 → 사용된 자격 증명 삭제로 이어지는 보안 중심의 Workflow를 구현합니다.
